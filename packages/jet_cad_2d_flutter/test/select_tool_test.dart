...
void main() {
  ...
  test('left-to-right encloses, right-to-left touches', () {
    ...
    final selection = SelectionController(doc);
    addTearDown(selection.dispose);
    final camera = cameraAt(1.0, const Offset(-50, 1300));
    final ctx = ToolContext(
        document: doc, index: index, camera: camera, selection: selection);

    final leftToRight = SelectTool();
    leftToRight.onPointerDown(ev(camera, const Offset(40, 290)), ctx);
    leftToRight.onPointerMove(ev(camera, const Offset(160, 310)), ctx);
    expect(leftToRight.phase, ToolPhase.dragging);
    expect(leftToRight.bandMode, BandMode.window);
    leftToRight.onPointerUp(ev(camera, const Offset(160, 310)), ctx);
    expect(selection.length, 1);
    expect(selection.contains(SelectionKey.root(inside)), isTrue);

    selection.clear();
    final rightToLeft = SelectTool();
    rightToLeft.onPointerDown(ev(camera, const Offset(160, 310)), ctx);
    rightToLeft.onPointerMove(ev(camera, const Offset(40, 290)), ctx);
    expect(rightToLeft.phase, ToolPhase.dragging);
    expect(rightToLeft.bandMode, BandMode.crossing);
    rightToLeft.onPointerUp(ev(camera, const Offset(40, 290)), ctx);
    expect(selection.length, 2);
    expect(selection.contains(SelectionKey.root(inside)), isTrue);
  ...
  test('a group is window-selected only when every leaf is enclosed', () {
    ...
    final selection = SelectionController(doc);
    addTearDown(selection.dispose);
    final camera = cameraAt(1.0, const Offset(-400, 400));
    final ctx = ToolContext(
        document: doc, index: index, camera: camera, selection: selection);

    // Band world box [486, 510] x [280, 320]: encloses the first leaf only.
    final window = SelectTool();
    window.onPointerDown(ev(camera, const Offset(86, 120)), ctx);
    window.onPointerMove(ev(camera, const Offset(110, 80)), ctx);
    expect(window.bandMode, BandMode.window);
    window.onPointerUp(ev(camera, const Offset(110, 80)), ctx);
    expect(selection.isEmpty, isTrue);

    final crossing = SelectTool();
    crossing.onPointerDown(ev(camera, const Offset(110, 80)), ctx);
    crossing.onPointerMove(ev(camera, const Offset(86, 120)), ctx);
    expect(crossing.bandMode, BandMode.crossing);
    crossing.onPointerUp(ev(camera, const Offset(86, 120)), ctx);
    expect(selection.length, 1);
  ...
  test('shift-band toggles', () {
    ...
    tool.onPointerDown(ev(camera, const Offset(280, 290), shift: true), ctx);
    tool.onPointerMove(ev(camera, const Offset(240, 310), shift: true), ctx);
    expect(tool.bandMode, BandMode.crossing);
    tool.onPointerUp(ev(camera, const Offset(240, 310), shift: true), ctx);
    expect(selection.length, 2);
    expect(selection.contains(SelectionKey.root(lineA)), isTrue);
    expect(selection.contains(SelectionKey.root(lineB)), isTrue);
