...
void main() {
  ...
  test(
    ...
    // ellipse's box, which reaches x = 4, while the narrow phase
    // deliberately approximates it by the circle of radius
    // 2 * sqrt(10) ~= 6.3246 (see distance.dart). A broad phase that were
    // only the indexed box would reject the band below before
    ...
    expect(rho, greaterThan(4.0),
        reason: "the fixture is only meaningful past the ellipse box's x = 4");
    expect(
        handlesOf(doc,
            leavesIn(index, bandAt(Vector2(rho, 0), 0.3), BandMode.crossing)),
        [circle]);
    expect(leavesIn(index, bandAt(Vector2(rho + 3, 0), 0.3), BandMode.crossing),
        isEmpty,
  ...
  test('a fill slot is never reported', () {
    ...
    final boundarySlot = doc.entities.slotOf(cmd.boundary.handle)!;
    expect(doc.entities.kindAt(fillSlot), EntityKind.fill);

    // Straddles the square's bottom edge, so the fill's box (which is the
    // boundary's box) is touched just as surely as the boundary itself.
    const edge = Aabb2.raw(4, -1, 6, 1);
    expect(leavesIn(index, edge, BandMode.crossing), [boundarySlot]);

    const around = Aabb2.raw(-5, -5, 15, 15);
    expect(leavesIn(index, around, BandMode.window), [boundarySlot],
  ...
  test('results are ascending by handle even when the tree order differs', () {
    final doc = DraftDocument.empty();
    // Three handles taken up front, then used out of order, so the R-tree's
    ...
    void line(Handle handle, double x) {
      doc.commands.execute(AddEntityCommand(
        record: EntityRecord(
          handle: handle,
          owner: doc.rootHandle,
        ...
        payload: GeometryPayload(
          coords: Float64List.fromList([x, 0, x + 1, 0]),
          scalars: Float64List(0),
        ),
      ));
    }

    line(c, 0); // lowest x, highest handle
    line(a, 40);
    line(b, 20);
    final index = SpatialIndex(doc);
    addTearDown(index.dispose);

    const band = Aabb2.raw(-5, -5, 50, 5);
    expect(handlesOf(doc, leavesIn(index, band, BandMode.window)), [a, b, c]);
    expect(handlesOf(doc, leavesIn(index, band, BandMode.crossing)), [a, b, c]);
  });
  ...
  test('instances come back ascending even when the tree order differs', () {
    ...
    const a = Handle(400), b = Handle(401), c = Handle(402);
    for (final (handle, x) in [(a, 200.0), (b, 0.0), (c, 100.0)]) {
      doc.commands.execute(AddNodeCommand(InstanceNode(
        handle: handle,
        parent: doc.rootHandle,
        transform: Transform2.translation(x, 0),
        definition: def,
        layer: ReservedHandles.layerZero,
      )));
    }
    ...
    const band = Aabb2.raw(-5, -5, 215, 5);
    expect(instancesIn(index, band, BandMode.crossing), [a, b, c]);
  ...
  test('a locked layer is skipped under picking, kept under all', () {
    ...
    final open =
        addEntity(doc, doc.rootHandle, EntityKind.line, [0, 0, 10, 0], []);
    final shut = addEntity(
        doc, doc.rootHandle, EntityKind.line, [0, 5, 10, 5], [],
        layer: locked);
    final index = SpatialIndex(doc);
    addTearDown(index.dispose);

    const band = Aabb2.raw(-5, -5, 15, 10);
    expect(
