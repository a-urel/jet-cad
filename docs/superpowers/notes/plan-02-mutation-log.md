...
### M-02b — drop the instance transform at `forEachInstanceInBand`'s own composition (entry of record)
file: packages/jet_cad_2d/lib/src/index/spatial_index.dart, `forEachInstanceInBand`: `child, root.transformOfInstance(node), mode, world, filter, 1` → `child, Transform2.identity(), mode, world, filter, 1` — the instance transform is dropped at the root level, where the spec's row's kill column places it
test: CI=true dart test test/index/band_query_test.dart
result: FIRED — `an instance is selected where its leaf lands after the transform` — `Expected: [400]` … `Actual: []` — this is the spec-named fixture: instance at (300, −200), 30°, ×1.5 (`placement`), placed directly under the root. Six further tests in the same file also went red: `an L-shaped block is not crossed by a band in the empty quadrant of its box`, `a grouped leaf inside a definition uses the group transform too`, `a window refuses an instance whose far leaf lies outside the band`, `a collapsed instance is judged by its image, not refused`, `instances come back ascending even when the tree order differs`, and `crossing and window agree with the brute-force arm on the generated corpus` (`Expected: Set:[437]` … `Actual: Set:[]`, BandMode.crossing trial 12).
restored: diff clean

M-02b (nested) — companion run, not a separate id: dropping the *recursive* composition instead (`_bandDescend`: `toWorld.multiply(index.transformOfInstance(node))` → `toWorld`, reached only for an instance nested two or more levels deep) was the mutation originally logged here in round 1 of review. Kept for the record: `CI=true dart test test/index/band_query_test.dart` → FIRED, but only via `crossing and window agree with the brute-force arm on the generated corpus` (`Expected: Set:[443]` … `Actual: Set:[]`, BandMode.window trial 18) — every hand-written fixture in this file places its instance directly under the root, so the single-level `placement` fixture stayed green under this variant; it is not the entry of record because the spec's row names the root-level call site, now fixed above. Restored: diff clean.
