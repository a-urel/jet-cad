...
**Layer fired in:** unit — `test/tile_fallback_test.dart`, `criterion 2 and
2c`, via the same command as M2. Re-fired a second time in Task 5's fix round
1 with the triangle-budget gate (below, M5) already in place, to confirm the
new gate did not change M3's kill path.
...
**Ruling: KILLED**, both before and after the triangle-budget gate existed.

**The fuller kill amendment 1 requires recorded here.** Under M3, **criterion
2b also reddens** — `differing: 417` against a bound of 60 — not only
criteria 2 and 2c as the plan's own text claimed. This figure is not in
`task-4-report.md` or `task-5-report.md`; it is recorded in this plan's
ledger (`.superpowers/sdd/2026-08-25-jet-cad-2d-plan-3h-pan-frame/progress.md`,
line 89: "Task 5: out of scope, carry to Task 8: under M3, criterion 2b also
reddens (`differing: 417` against a bound of 60). The plan's M3 record claims
only criteria 2 and 2c; the mutation log should record the fuller kill.") and
restated verbatim in this task's own brief (`task-8-brief.md`, controller
amendment 1, item 3). Task 8a did not re-run this measurement — see "Figures
not independently reproduced by Task 8a" at the end of this log.
...
Per this task's scope, no mutant was re-fired and no new measurement was
taken — every figure above is read from
`.superpowers/sdd/2026-08-25-jet-cad-2d-plan-3h-pan-frame/task-4-report.md`,
`task-5-report.md`, or (for the one figure below) the plan's ledger, per the
brief's instruction to read rather than re-run.

- **M3's criterion 2b figure, `differing: 417` against a bound of 60**, is
  not in `task-4-report.md` or `task-5-report.md`. It is recorded in
  `progress.md` (line 89) and restated in `task-8-brief.md`'s controller
  amendment 1. Task 8a took both as given, per the brief's own instruction,
  and did not re-run the M3 mutation to reproduce it a third time.
- **M5's first-fired result, `+372 ~1` green against the whole package**, is
  likewise from `progress.md` (line 79) rather than from either named task
  ...
  reviewer's original discovery run.
