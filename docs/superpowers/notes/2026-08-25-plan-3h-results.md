...
them explain the miss; the miss is real. It lands close to gate (2.35 against
2.4, roughly a 2% shortfall in the ratio) and inside the spread the six
individual runs already show (narrowed p95 alone ranges 13.43–19.86 ms, a
6.4 ms spread on one arm), so it reads as this machine's variance at the
boundary the spike's single 2.59x sample did not happen to land at, not as
evidence the narrowing stopped working — criteria 4, 5 and the M4 unit result
below all corroborate that the narrowing is doing real, large work. It is
reported as a MISS regardless, per instruction.
...
`100663296` bytes (96.00 MiB) — worth flagging beside criterion 7 even though
criterion 7 itself is scored on the narrowed arm's default-step figure, not
this sweep.
...
kill it. M4 also walks the whole viewport — a different mutation from M5
(M4 additionally keeps the narrow clip; M5 doesn't touch the clip either, per
the mutation table, so the two differ only in exactly what M5's own name
says: M4 narrows the clip but not the query, M5 grows the query while leaving
the clip as-is — the same triangle-count symptom results from either,
because the gate only sees geometry, not pixels). The same gate that was
built to kill M5 kills M4 as well.
...
|---|---|---|---|
| 3 | `tile pan` p95 ratio, M4/narrowed, ≥ 2.4x | **MISS** | 2.35x (37.59 / 15.99 ms) |
...
| 6 | `tile hold` p50 ≤ 2.0ms, p95 ≤ 2.5ms | **PASS** | median p50=1.45ms, median p95=1.79ms (one individual run's raw p95, 2.77ms, exceeds 2.5ms; the three-run median does not) |
...
| M4 | dies on criterion 3 and (newly) on the widget suite | **DIES — doubly** | widget suite RED (`tile_fallback_test.dart`, criterion 2/2c); device ratio 2.35x (short of 2.4 but far from M4's own ≈1.0-against-itself framing) |

**Exit-gate note:** criterion 3, this task's headline, misses its gate
(2.35 against ≥ 2.4). Per instruction this is reported as measured, not
adjusted or re-run to chase the threshold. Six device runs support it (three
narrowed, three M4), all taken in one contiguous session with Low Power Mode
confirmed off before and after each.
