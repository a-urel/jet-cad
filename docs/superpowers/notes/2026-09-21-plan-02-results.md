...
**Twelve tasks done, `3fedeb9..afe7d64` (Tasks 1–11); this task (the results
note, spec amendments, STATUS and the roadmap) lands on top. NOT merged — the
...
```
00:10 +765 ~1 -5: Some tests failed.
...
Exit 1 — **765 pass, 1 pre-existing skip, and exactly the same five
...
--set-exit-if-changed .`: `Formatted 136 files (0 changed) in 0.25 seconds.`
...
12 tests (indices 0–11), including
...
`jet_cad_2d_flutter` bar is therefore "765 pass, 1 skip, those same five
...
| 5 | Shift toggles; Escape cancels a band or clears; Delete removes through the command log on one key-down; undo restores geometry, not selection | **PASS** | `select_tool_test.dart`: `click replaces, shift-click toggles`, `Escape during a band drops it, selection untouched; cancel returns to idle`; `interaction_layer_test.dart`: `one Delete press is one remove` |
...
6. Delete removes the selection, and undo brings the geometry back. **OWED —
...
5. Escape cancels a band or clears, in both browsers. **OWED — not looked
   at; the human looks after this branch is presented.**
6. Delete removes and undo restores the geometry, in both browsers. **OWED —
   not looked at; the human looks after this branch is presented.**
7. Undo restores geometry, not selection, in both browsers. **OWED — not
   looked at; the human looks after this branch is presented.**
...
- **`worldPointOf` allocates one `Vector2` per selected point per frame**
  (accepted, Task 8 ruling) — points are rare in a floor plan and the
  per-entity allocation rule is about entities walked, not keys selected.
